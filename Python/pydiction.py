...
__author__ = "Luffy"
__version__ = "1.2"
__copyright__ = "Copyright (c)Luffy"
...
def get_submodules(module_name, submodules):
    ...
    mod_attrs = dir(imported_module)  # View all properties and methods
...
def my_import(name):
    ...
    mod = __import__(name)    # __import__(name) is (import mod)
    components = name.split('.')
    for comp in components[1:]:
        mod = getattr(mod, comp)  # mod : comp or None
    return mod
...
def remove_duplicates(seq, keep=()):
    ...
    """
    seq2 = []
    seen = set()   # seen : unordered and non_redundant
    for i in seq:
        if i in keep:
            seq2.append(i)
            continue
            ...
            for module_name in sys.argv[1:]:
                for line in file_lines:
                    if line.find('--- import %s ' % module_name) != -1:
                        ...
                        sys.argv.remove(module_name)  # clear the module_name in sys.argv
                        break
            f.close()
            ...
            if os.path.exists(PYDICTION_DICT_BACKUP):
                ...
                if answer:
                    print "Backing up old dictionary to: %s" % \
                          PYDICTION_DICT_BACKUP
                    try:
                        shutil.copyfile(PYDICTION_DICT, PYDICTION_DICT_BACKUP)  # exist:override
                    except IOError, err:
                ...
                try:
                    shutil.copyfile(PYDICTION_DICT, PYDICTION_DICT_BACKUP)    # not exist:create
                except IOError, err:
                    print "Couldn't back up %s. %s" % (PYDICTION_DICT, err)
    else:
